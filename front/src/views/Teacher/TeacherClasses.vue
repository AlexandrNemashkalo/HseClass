  ...
  <div style="width: 100%">
          ...
          >
            <v-card class="m-2" elevation="4">
              <v-img
                  ...
                  @click="$router.push('/teacher/class/'+card.id)"
    ...
    >
      <v-card>
                              ...
                              <v-card-title>
                                <span class="headline">Редактирование класса</span>
                              </v-card-title>
                              <v-card-text>
                                <v-form ref="formEdit"
                                        v-model="validEdit"
    ...
    >
      ...
      <v-card>
        <v-card-title>
          <span class="headline">Создание класса</span>
        </v-card-title>
        <v-card-text>
          <v-form ref="form"
                  v-model="valid"
...
export default {
  ...
  data(){
    return{
      ...
      titleRules: [
        v => !!v || 'Название класса не указано'
      ],
      isEditing: false,
      model: null,
      dialog: false,
  ...
  async created() {
    ...
    await this.$store.dispatch("GetClassesTeacher")
  },

  methods:{
    showEdit(card){
      this.editedForm.title = card.title;
      this.editedForm.id = card.id;
      this.isEdit = true;
    },

    async editClass(){
      if(this.$refs.formEdit.validate()){
        await this.$store.dispatch("EditClassTeacher",{id:this.editedForm.id,data:{title:this.editedForm.title}})
        this.isEdit = false;
      }
    },
    ...
    async addClass(){
      if(this.$refs.form.validate()){
        await this.$store.dispatch("AddClassTeacher",{title:this.form.title})
        this.dialog = false;
      }
    },
    showCode(card){
    ...
    async deleteClass(id){
      ...
      Swal.fire({
        ...
        text: "После удаление класса удаляться все назначенные лабораторные работы",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          await this.$store.dispatch("DeleteClassTeacher",id)
          Swal.fire(
              'Удалено!',
